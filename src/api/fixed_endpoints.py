"""
Fixed Advanced API Endpoints for OpenManus-Youtu Integrated Framework
Enhanced endpoints with monitoring, health checks, and system management - Fixed version
"""
...
async def health_check():
    ...
    try:
        ...
        if health_status["gemini"]["status"] != "configured":
...
async def get_metrics():
    """Get system metrics."""
    try:
        # Basic metrics without psutil dependency
        metrics = {
            "timestamp": datetime.now().isoformat(),
            "application": {
                "uptime": str(datetime.now() - system_metrics["start_time"]),
