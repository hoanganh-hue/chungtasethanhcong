#!/usr/bin/env python3
"""
Fixed test script for Gemini 2.0 Flash with correct API format
...
class FixedGeminiClient:
    """Fixed Gemini API client with correct format."""
    ...
    async def generate_content(
        ...
        """Generate content using Gemini API with correct format."""
        try:
            ...
            # Convert messages to correct API format
            contents = []
            for msg in messages:
                # Only include user messages in contents
                if msg.role == "user":
                    content = {
                        "parts": msg.parts
                    }
...
class UnifiedGeminiAgent:
    """Unified Gemini Agent with real API integration."""
    
    def __init__(self, name: str, description: str, gemini_config: GeminiConfig):
        self.name = name
        self.description = description
        self.gemini_config = gemini_config
        self.client = FixedGeminiClient(gemini_config)
    ...
    async def process_message(
        ...
        try:
            ...
            api_messages.append(GeminiMessage(
                role="user",
                parts=[{"text": system_prompt}],
                timestamp=datetime.now()
            ))
            
            # Add recent context messages (last 3)
            ...
            response_chunks = []
            async for chunk in self.client.generate_content(api_messages, stream=stream):
...
async def test_cccd_agent():
    """Test CCCD agent with real API."""
    print("🧪 Testing CCCD Agent with Gemini 2.0 Flash...")
    ...
    try:
        ...
        if response_chunks:
            response = "".join(response_chunks)
            print(f"✅ CCCD generation response: {response[:300]}...")
        else:
        ...
        if response_chunks:
            response = "".join(response_chunks)
            print(f"✅ CCCD check response: {response[:300]}...")
        else:
            print("❌ No response received")
        ...
        if response_chunks:
            response = "".join(response_chunks)
            print(f"✅ Tax lookup response: {response[:300]}...")
        else:
            print("❌ No response received")
        
        # Cleanup
        await agent.close()
        print("\n✅ CCCD Agent test completed successfully")
        return True
        
    except Exception as e:
        print(f"❌ CCCD Agent test failed: {e}")
        return False

async def test_general_agent():
    """Test general purpose agent with real API."""
    print("\n🧪 Testing General Purpose Agent with Gemini 2.0 Flash...")
    ...
    try:
        ...
        if response_chunks:
            response = "".join(response_chunks)
            print(f"✅ General conversation response: {response[:300]}...")
        else:
            print("❌ No response received")
        ...
        if response_chunks:
            response = "".join(response_chunks)
            print(f"✅ Function calling response: {response[:300]}...")
        else:
            print("❌ No response received")
        
        # Cleanup
        await agent.close()
        print("\n✅ General Agent test completed successfully")
        return True
...
async def main():
    """Run all fixed Gemini agent tests."""
    print("🚀 Starting Fixed Unified Gemini Agent Tests")
    print("=" * 60)
    ...
    # Run all tests
    test_results.append(await test_cccd_agent())
    test_results.append(await test_general_agent())
    test_results.append(await test_agent_performance())
    
    # Summary
    print("\n" + "=" * 60)
    print("🎉 Fixed Unified Gemini Agent Tests Completed!")
    ...
    if passed_tests == total_tests:
        print("🎉 All tests passed! Fixed Unified Gemini Agent integration is working perfectly!")
    else:
        print("⚠️  Some tests failed. Please check the error messages above.")
    
    print("\n🔧 Features Tested:")
    print("✅ CCCD Agent with Gemini 2.0 Flash")
    print("✅ General Purpose Agent")
    print("✅ Function calling simulation")
    print("✅ Performance testing")
    print("✅ Context management")
    print("✅ Vietnamese language support")
    print("✅ Fixed API format")
